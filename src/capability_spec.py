    ...
    def _create_default_operations() -> list[OperationSpec]:
        return [
            ...
            OperationSpec(
                ...
                description=(
                    "Рассчитать взвешенное среднее (Weighted Mean), сворачивая КОНКРЕТНЫЙ вопрос-шкалу в одно число. "
                    "Логика работы: "
                    "1. Берет уровень мультииндекса, указанный в аргументе `question`. "
                    "2. Преобразует ответы этого уровня в веса согласно `scale`. "
                    "3. Если есть другие уровни индекса (например, 'Регион'), группирует данные по ним. "
                    "Используйте это, чтобы получить таблицу средних значений (например, средний чек по регионам)."
                ),
                inputs={
                    "pivot_table": "DataFrame — Таблица с данными (MultiIndex или Index).",
                    "question": "str — Название уровня индекса (вопроса), который нужно усреднить. Должен совпадать с именем уровня в таблице (или быть его частью).",
                    "scale": "Dict[str, float] — Словарь весов {'Вариант ответа': Число}. Ключи должны присутствовать в выбранном уровне `question`."
                },
                outputs=["average_table"],
                example={
                    "pivot_table": "pivot_region_income",
                    "question": "[N15] Какую долю дохода тратите?", 
                    "scale": {'10-20%': 15, '20-30%': 25, 'Более 30%': 35}
                }
            )
        ]
